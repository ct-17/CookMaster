...
def food_list(request):
    query = request.GET.get("q", None)
    qs = PostModel.objects.all().order_by('-publish_date').filter(kind="food_list")
    paginator = Paginator(qs, 12)
    page = request.GET.get('page')
    ...
    template = "blog/food_list.html"
    return render(request, template, context)

def list_food_all(request):
    query = request.GET.get("q", None)
    qs = PostModel.objects.all().order_by('-publish_date').filter(kind="list_food_all")
    paginator = Paginator(qs, 12)
    page = request.GET.get('page')
